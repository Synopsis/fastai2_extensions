...
class ClassificationInterpretationEx(ClassificationInterpretation):
    ...
    def compute_label_confidence(self, df_colname: Optional[str] = "fnames"):
        ...
        for label, df in self._preds_df_each.items():
            ...
            self.preds_df_each[label]["accurate"] = sort_desc(
                self.preds_df_each[label]["accurate"], label
            )
            self.preds_df_each[label]["inaccurate"] = sort_desc(
                self.preds_df_each[label]["inaccurate"], label
            )
            assert len(self.preds_df_each[label]["accurate"]) + len(
                self.preds_df_each[label]["inaccurate"]
            ) == len(df)
