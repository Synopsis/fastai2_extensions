...
def compare_venn(
    ...
    * conf_level: a single number or a tuple of (min,max) to filter the prediction confidence
    ...
    if len(interps) <= 3:
        ...
        if title is not None:
            ...
            if isinstance(conf_level, tuple):
                filler = f"Between {conf_level[0]}-{conf_level[1]}"
            else:
                if mode == "accurate":
                    filler = f"Above {conf_level}"
                else:
                    filler = f"Below {conf_level}"
            if mode == "accurate":
                fig.suptitle(
                    f"Model Agreement - {mode.capitalize()} {filler} % Confidence"
                )
            if mode == "inaccurate":
                fig.suptitle(
                    f"Model Agreement - {mode.capitalize()} {filler} % Confidence"
    ...
    if return_common:
        res_set = {
            label: intersection(sets_dict[label]) for label in interps[0].dl.vocab
        }

    if return_fig and return_common:
        return (fig, res_set)
    if return_fig and not return_common:
        return fig
    if return_common and not return_fig:
        return res_set
