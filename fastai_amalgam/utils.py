...
# Cell
def open_image(
    fname: Union[str, Path], size: tuple = None, as_array=False, as_tensor=False
):
    "Open a _single_ image from disk, optionally resize and/or convert to array"
    img = PIL.Image.open(fname).convert("RGB")
    if isinstance(size, float):
        size = L(img.size).map(lambda x: int(x * size))
    if size is not None:
        img = img.resize(size)
    if as_array or as_tensor:
        return img2arraylike(img, as_array, as_tensor)
    return img
...
# Cell
def make_img_grid(
    images: Union[List[str], List[PILImage], List[PIL.Image.Image]],
    ...
    """
    if not isinstance(images, (list, L)):
        raise TypeError(
            f"Expected a list of (paths,PILImage,PIL.Image.Image) objects, got {type(images)} instead"
        )
    ...
    * fsize_div_factor: Set font size to img.width/fsize_div_factor
    ...
    if font_path is not None:
        ...
        if font_size is not None:
            warnings.warn(
                f"`font_size` cannot be used when not using a custom font passed via `font_path`"
            )
    img = (
        self  # this allows us to easily pull out the function without `@patch` support
    )
    draw = ImageDraw.Draw(img)
    if font_size is None:
        font_size = int(img.width / fsize_div_factor)
    ...
    if not location in valid_locations:
        raise ValueError(f"`location` must contain 'bottom' or 'top', got {location}")
