...
from albumentations import (
    RandomBrightness,
    RandomContrast,
    CLAHE,  # Lighting Transforms
    ToSepia,
    ToGray,  # Color Tones
    RGBShift,
    HueSaturationValue,
    ChannelShuffle,  # Color Channels
    Blur,
    MotionBlur,
    GaussianBlur,  # Blurs
    MedianBlur,
    IAAEmboss,  # Style Transfer-Esque
    GaussNoise,
    IAAAdditiveGaussianNoise,  # Noise
    JpegCompression,
    Posterize,  # Noise
...
# Cell
def albu_augment(aug, img):
    return aug(image=np.array(img))["image"]
...
class AlbumentationsWrapper(Transform):
    ...
    def encodes(self, img: PIL.Image.Image):
        return self._encodes(img)

    def encodes(self, img: PILImage):
        return self._encodes(img)

    def encodes(self, o: (str, Path)):
        return self._encodes(PILImage.create(o))


# Cell

# lean heavily towards Motion Blur
BlurringTfms = OneOf([MotionBlur(blur_limit=(15, 15), p=0.8), Blur(p=0.2)], p=0.5)

StyleTfms = OneOf(
    [MedianBlur(blur_limit=(3, 7), p=0.6), IAAEmboss(strength=(0.2, 0.99), p=0.4)],
    p=0.3,
)
...
ColorChannelTfms = OneOf(
    [ChannelShuffle(), HueSaturationValue(val_shift_limit=5), RGBShift()], p=0.3
)

LightingTfms = OneOf(
    [RandomContrast(p=0.1), RandomBrightness(p=0.1), CLAHE(p=0.8)], p=0.3
)
