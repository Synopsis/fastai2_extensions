...
class FocalLoss(nn.Module):
    ...
    def __init__(self, alpha=0.25, gamma=2, reduction="none", pos_weight=None):
    ...
    def forward(self, inputs, targets):
        BCE_loss = F.binary_cross_entropy_with_logits(
            inputs, targets, reduction=self.reduction, pos_weight=self.pos_weight
        )
...
class FocalLossFlat(BaseLoss):
    @use_kwargs_dict(
        keep=True, alpha=0.25, gamma=2.0, reduction="none", pos_weight=None
    )
